...
node combine(node a, node b){
  return node(a.xor_seq^b.xor_seq, a.or_seq|b.or_seq, a.and_seq&b.and_seq, min(a.mn, b.mn), max(a.mx, b.mx));
}
...
void update(int i, int l, int r, int p, int var){
  ...
  if(l == p and r == p){
    arr[p] = var;
    segtree[i] = node(arr[p], arr[p], arr[p], p, p);
    return;
  }
  int md = (l+r)/2, nxt = 2*i;
...
node query(int i, int l, int r, int x, int y){
  if(l > y or r < x){
    return node(0, 0, (1LL<<30)-1, maxn-1, maxn-2);
  }
  if(l >= x and r <= y){
    return segtree[i];
  }
...
int main(){
  ...
  while(q--){
    ...
    cin >> op >> x >> y;
    if(op == 'x'){
      node a = query(1, 0, n-1, x-1, y-1);
      cout << (a.xor_seq == 0 ? arr[a.mx] : -1) << "\n";
    }
    else if(op == 'o'){
      int id = query(1, 0, n-1, x-1, y-1).mx;
      ll L = query(1, 0, n-1, x-1, id-1).or_seq, R = query(1, 0, n-1, id+1, y-1).or_seq;
      ll OR = L|R;
      cout << (OR == arr[id] ? arr[id] : -1) << "\n";
    }
    else if(op == 'a'){
      int id = query(1, 0, n-1, x-1, y-1).mn;
      ll L = query(1, 0, n-1, x-1, id-1).or_seq, R = query(1, 0, n-1, id+1, y-1).or_seq;
      ll AND = L&R;
